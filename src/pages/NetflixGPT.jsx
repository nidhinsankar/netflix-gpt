...
const NetflixGPT = () => {
  ...
  const onSubmit = async () => {
    const gptQuery =
      "act as a movie recommendation system suggest me based on " +
      searchRef.current.value +
      "only five movies name in comma separated movies like given in examples ahead. example result: nayakan, master,amar akbar anthony, Anwar, puthiya mugam";
  ...
  return (
    <div className="">
      <img
        src={banner}
        alt="netflix-banner"
        className="fixed -z-20 h-screen w-full object-cover"
      />
      <div className="flex flex-col md:flex-row justify-center items-center w-[95%] mx-auto">
        <input
          className="mt-[30%] md:mt-[10%] focus:outline-none focus:ring-4 ring-net-red  font-medium text-lg py-2 px-3 shadow-md rounded-md"
          placeholder="Search....."
          type="text"
          name="searchGPT"
          id="search"
        ...
        />
        <button
          className="mt-4 md:mt-[10%] py-2 ml-2 px-4 bg-net-red shadow-md rounded-md"
          onClick={() => onSubmit()}
        >
          Search
        </button>
